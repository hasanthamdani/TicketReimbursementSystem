...
export interface EmployeeContextType {
    ...
    logout: () => void;
    register: (username: string, password: string, isManager:boolean) => void;
}
...
//The functional component will provide context for each of the Children
    ...
    const login = (username:string, password:string) => {setEmployee({username, password}); setIsAuth(true);};

    const logout = () => {setEmployee(null); setIsAuth(false);};

    const register = (username:string, password:string, isManager:boolean) => {setEmployee({username, password}); setIsAuth(true); setIsManager(isManager)};
